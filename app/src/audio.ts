...
export class TerminalAudio {
  ...
  // Background music removed from this deployment; keep the signature so
  // call sites and the settings flow stay untouched.
  prepareMusic() {
    this.musicData ??= [];
    return Promise.resolve(this.musicData);
  }
  restartBoot() {
    this.stopEffects();
    this.bootTime = 6.76;
